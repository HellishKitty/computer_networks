...
def by_surname(worker):
    return worker.surname


def by_position(worker):
    return worker.position


def by_salary(worker):
...
def sort(r):
    ...
    if r == 'surname':
        ...
        notes = sorted(notes, key=by_position)
    elif r == 'salary':
        notes = sorted(notes, key=by_salary)
    elif r == 'birth':
